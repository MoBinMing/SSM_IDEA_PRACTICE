    ...
    <div class="row ">
      <div class="col-md-12 p-0">
        <ul class="nav nav-tabs">
          <li class="nav-item" >
            <a href="" class="active nav-link" data-toggle="tab" data-target="#adminApi">
              管理员
              <span class="badge badge-primary badge-pill" >${fn:length(adminApis)}</span>
            </a>
          </li>
          <li class="nav-item" >
            <a href="" class="nav-link" data-toggle="tab" data-target="#teacherApi">
            教师端&nbsp;
              <span class="badge badge-primary badge-pill" >${fn:length(teacherApis)}</span>
            </a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="" data-toggle="tab" data-target="#studentApi">
              学生端
        ...
        <div class="tab-content mt-2">
          <div class="tab-pane fade show active" id="adminApi" role="tabpanel">
            <div class="row ">
              <div class="col-md-12">
                <div class="row m-0 p-3">
                  <div class="col card-header p-0 col-9 col-sm-6 col-md-12 col-lg-6">
                    <div class="input-group">
                      <input id="searchAdminApisVal" placeholder="Search Admin API" type="text" class="form-control input-lg"
                             onkeyup="searchApis('admin');">
                      <span class="input-group-addon btn btn-primary" onclick="searchApis('admin');">
                          <i class="fa fa-search"></i>&nbsp;搜索</span>
                    </div>
                  </div>
                </div>
                <div class="container" id="adminApiContainer">
                  <c:forEach var="api" items="${adminApis}">
                    <div class="row mb-4" >
                      <div id="adminApiCol1${api.id}" class="col-md-7">
                        <div class="card">
                          <div class="row m-0 card-header">
                            ...
                            <c:if test="${api.type=='POST'}">
                              <button onclick="$('#adminApiResult${api.id}').html('${api.requestData}')"> 请求的json </button>

                            </c:if>
                            <c:if test="${api.type=='GET'}">
                              <button onclick="$('#adminApiResult${api.id}').html('${api.requestData}')"> 请求的后缀 </button>

                            </c:if>
                            <button class="ml-2 mr-2" onclick="$('#adminApiResult${api.id}').html('${api.succeedHtml}')"> 成功示例 </button>

                            <button onclick="$('#adminApiResult${api.id}').html('${api.defeatedHtml}')"> 失败示例 </button>
                          </div>
                          <div class="card-body">
                            <blockquote class="blockquote mb-0">
                              <span class="row m-0 p-0">
                                <p class="col m-0 p-0" >url:</p>
                                <input value="复制链接" onclick="$('#adminUrl${api.id}').select();document.execCommand('Copy');" class="" type="button">
                              </span>

                              <input class="type=&quot;text&quot; w-100" size="30" id="adminUrl${api.id}"  value="${api.url}" >
                              <p class="m-0"> 简介： </p>
                      ...
                      <div id="adminApiResult${api.id}" class="textarea col-md-5" contenteditable="true">
                        <br />
                      </div>
                    </div>
                  </c:forEach>
          ...
          <div class="tab-pane fade " id="teacherApi" role="tabpanel">
            <div class="row ">
              <div class="col-md-12">
                <div class="row m-0 p-3">
                  <div class="col card-header p-0 col-9 col-sm-6 col-md-12 col-lg-6">
                    <div class="input-group">
                      <input id="searchTeacherApisVal" placeholder="Search API" type="text" class="form-control input-lg"
                             onkeyup="searchApis('teacher');">
                      <span class="input-group-addon btn btn-primary" onclick="searchApis('teacher');">
                          <i class="fa fa-search"></i>&nbsp;搜索</span>
                    </div>
                  </div>
                </div>
                <div class="container" id="teacherApiContainer">
                  <c:forEach var="api" items="${teacherApis}">
                    <div class="row mb-4" >
                      <div id="teacherApiCol1${api.id}" class="col-md-7">
                        <div class="card">
                          <div class="row m-0 card-header">
                            <div class="col">
                              <font color="#212529">
                              ...
                              <c:if test="${api.type=='POST'}">
                                  <button onclick="$('#teacherApiResult${api.id}').html('${api.requestData}')"> 请求的json </button>

                              </c:if>
                              <c:if test="${api.type=='GET'}">
                                  <button onclick="$('#teacherApiResult${api.id}').html('${api.requestData}')"> 请求的后缀 </button>

                              </c:if>
                            <button class="ml-2 mr-2" onclick="$('#teacherApiResult${api.id}').html('${api.succeedHtml}')"> 成功示例 </button>

                            <button onclick="$('#teacherApiResult${api.id}').html('${api.defeatedHtml}')"> 失败示例 </button>
                          </div>
                          <div class="card-body">
                            <blockquote class="blockquote mb-0">
                              <span class="row m-0 p-0">
                                <p class="col m-0 p-0" >url:</p>
                                <input value="复制链接" onclick="$('#teacherUrl${api.id}').select();document.execCommand('Copy');" class="" type="button">
                              </span>

                              <input class="type=&quot;text&quot; w-100" size="30" id="teacherUrl${api.id}"  value="${api.url}" >
                              <p class="m-0"> 简介： </p>
                              <footer class="blockquote-footer"><cite title="info"> ${api.info}</cite></footer>
                            </blockquote>
                          </div>
                      ...
                      <div id="teacherApiResult${api.id}" class="textarea col-md-5" contenteditable="true">
                        <br />
                      </div>
                    </div>
                  </c:forEach>
          ...
          <div class="tab-pane fade" id="studentApi" role="tabpanel">
            <div class="row ">
              <div class="col-md-12">
                <div class="row m-0 p-3">
                  <div class="col card-header p-0 col-9 col-sm-6 col-md-12 col-lg-6">
